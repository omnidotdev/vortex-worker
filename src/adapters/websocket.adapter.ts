...
export class WebSocketAdapter implements EventAdapter {
  ...
  #connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        // Bun's WebSocket accepts headers in the second arg at runtime,
        // but the TS type only allows string | string[], so we cast
        this.ws = this.#config.authHeaders
        ...
        this.ws.addEventListener("message", (event) => {
          ...
          const normalizedEvent: NormalizedEvent = {
            ...
            data,
            metadata: {
              idempotencyKey: `ws-${this.#config.url}-${crypto.randomUUID()}`,
              timestamp: new Date(),
            },
          };

          this.handler(normalizedEvent).catch((err) => {
            logger.error("WebSocket event handler failed", {
              url: this.#config.url,
