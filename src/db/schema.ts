...
export const workflowRunTable = pgTable(
  ...
  (table) => [
    ...
    index("workflow_run_created_at_idx").on(table.createdAt),
  ],
);
...
export const eventLogTable = pgTable(
  ...
  (table) => [
    index("event_log_organization_id_idx").on(table.organizationId),
    index("event_log_type_idx").on(table.type),
    index("event_log_source_idx").on(table.source),
    index("event_log_correlation_id_idx").on(table.correlationId),
    index("event_log_recorded_at_idx").on(table.recordedAt),
  ],
);
