...
const deliverToSubscription = async (
  ...
  try {
    ...
    // Defense-in-depth: validate URL scheme and target before delivery
    ...
    if (isBlockedUrl(targetUrl)) {
      throw new Error(
        `Blocked delivery to private/internal address: ${targetUrl.hostname}`,
      );
    }
...
/**
 * Retry poller that processes pending deliveries whose retry time has arrived.
    ...
    try {
      ...
      for (const delivery of pendingDeliveries) {
        try {
          ...
          // Defense-in-depth: validate URL scheme and target before retry delivery
          ...
          if (isBlockedUrl(retryUrl)) {
            throw new Error(
              `Blocked delivery to private/internal address: ${retryUrl.hostname}`,
            );
          }
