 ...
 * Rejects names containing anything other than alphanumeric, underscores, and dots.
 */
...
const buildWhere = (
  ...
  for (const [key, value] of Object.entries(where)) {
    ...
    if (value === null) {
      ...
      const opStr = op.op ?? "=";
      conditions.push(`${safeKey} ${opStr} $${idx++}`);
...
const createTable = async (
  ...
  try {
    ...
    const columnDefs = input.columns.map((col) => {
      const parts = [sanitizeIdentifier(col.name), typeMap[col.type] ?? "TEXT"];
    ...
    const sql = `CREATE TABLE ${ifNotExists}${safeTable} (${columnDefs.join(", ")})`;
...
const update = async (
  ...
  try {
    ...
    const setClauses = dataKeys.map(
      (col, i) => `${sanitizeIdentifier(col)} = $${i + 1}`,
    );
...
const deleteRows = async (
  ...
  try {
    ...
    const sql = `DELETE FROM ${sanitizeIdentifier(input.table)} ${whereClause}`;
